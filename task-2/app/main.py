...
def main():
    ...
    for user_id in list(USERS["user_id"]):
        ...
        logger.info(f"Connected to Postgres database to update last_emailed for user_id: {user_id}")
        ...
        created_user = USERS.loc[USERS["user_id"] == user_id, "created_at"].values[0]
